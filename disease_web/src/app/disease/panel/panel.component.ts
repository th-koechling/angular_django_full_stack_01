...
export class PanelComponent implements OnInit {
  ...
  displayedColumns: string[] = ['id', 'name', 'genes', 'delete'];
  ...
  ngOnInit(): void {
    ...
    forkJoin([panels, diseasePanels, genes]).subscribe(
      ([panelData, diseasePanelData, geneData]) => {
        ...
        console.log("GENES: ", this.geneList);
        //this.diseaseService.createPanel({id:0, name:'TTTest Panel', genes: [this.geneList[0]]}).subscribe((data) => {
        //  console.log("Test panel created:", data);
        //} );
      } 
    )
  }
  ...
  addUpdatePanel(panel: Panel) {
    ...
    console.log("With genes: ", panel.genes);
    console.log("Genes preselected: ", this.genesPreSelect);
    if (panel.id !== 0) {
  ...
  geneSelectionChange(selectedGenes: Gene[]) {
    this.panel.genes = selectedGenes;
  }
